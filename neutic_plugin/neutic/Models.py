...
import numpy as np
import neutic_plugin.neutic.Constants as Constants
from neutic_plugin.neutic.Layers import SelfAttention
...
class NeuTic(nn.Module):
    ...
    #                不同数值的数量 用于训练的包数 输出几分类
    def __init__(self, n_src_vocab, h, grained, d_word_vec=512, d=512, d_inner=2048, L=2, n_head=8, d_k=64, d_v=64, dropout=0.1):

        super().__init__()

        assert d == d_word_vec, 'To facilitate the residual connections, the dimensions of all module outputs shall be the same.'
    ...
    def forward(self, src_seq, src_pos):
        ...
        # wid_seq = torch.tensor(wid_seq)
        # flag_seq = torch.tensor(flag_seq)
